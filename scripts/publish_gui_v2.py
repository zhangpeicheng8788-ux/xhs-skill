...
class PublishGUI:
    def __init__(self, default_notes_dir=None, start_from=1, wait_minutes=20):
        ...
        # 创建主窗口
        self.root = tk.Tk()
        self.root.title("小红书笔记批量发布工具 V2.0")
    ...
    def setup_ui(self):
        ...
        self.path_hint_label = tk.Label(
            path_frame,
            text="提示：选择包含笔记的文件夹（如 D:\\jieyue_work）",
    ...
    def detect_notes(self):
        ...
        # 显示检测结果
        self.notes_count_label.config(text=f"笔记数量: {len(note_dirs)} 个", fg='green')
        ...
        for i, note_dir in enumerate(note_dirs, 1):
            ...
            try:
                ...
                total_images = min(cover_count + card_count, 9)  # 小红书最多9张
        ...
        self.progress_label.config(text=f"检测到 {len(note_dirs)} 个笔记，点击[开始发布]按钮启动任务")
        
        messagebox.showinfo("检测完成", f"递归检测完成！\n\n共找到 {len(note_dirs)} 个有效笔记\n\n可以开始发布了！")
    ...
    def publish_task(self):
        ...
        try:
            ...
            if self.start_from > 1:
                note_dirs = note_dirs[self.start_from-1:]
            ...
            for i, note_dir in enumerate(note_dirs, 1):
                ...
                # 检查是否已发布
                publish_record = os.path.join(note_dir, 'publish_record.json')
                ...
                try:
                    ...
                    if isinstance(result, dict):
                        ...
                        if note_id:
                            ...
                            with open(publish_record, 'w', encoding='utf-8') as f:
            ...
            messagebox.showinfo("完成", f"批量发布任务完成！\n\n成功发布: {published_count}/{total} 篇")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='小红书笔记批量发布 GUI V2.0')
